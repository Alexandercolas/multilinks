...
export default async function PublicProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (data) {
    const [{ data: links }, { data: hasPro }, { data: linkLimit }, { data: authData }] = await Promise.all([
      supabase.from("links").select("id,title,url,active,icon,section_title,description,featured").eq("profile_id", data.id).eq("active", true).order("position"),
    ...
    const profile: Profile = {
      ...
      links: (links ?? [])
        ...
        .map((link) => ({ ...link, icon: link.icon ?? undefined, sectionTitle: link.section_title ?? undefined, description: link.description ?? undefined, featured: Boolean(link.featured) })),
    };
