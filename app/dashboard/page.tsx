...
import { ChevronDown, Crown, Eye, GripVertical, ImagePlus, LogOut, MousePointerClick, Plus, ShieldCheck, Trash2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#090b0d] text-white">
    ...
    <header className="relative border-b border-white/10 bg-[#0d1014]/80 px-4 py-3 backdrop-blur-xl lg:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between"><Link href="/" aria-label="Volver al inicio" className="text-white"><Logo/></Link><nav aria-label="Acciones de la cuenta" className="flex items-center gap-2"><Link href={`/${profile.username}`} className="inline-flex items-center gap-2 rounded-xl bg-lime px-3 py-2.5 text-sm font-black text-ink transition hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(201,255,88,.15)] motion-reduce:transform-none motion-reduce:transition-none"><Eye size={17}/><span className="hidden sm:inline">Ver mi perfil</span></Link>{isAdmin ? <Link href="/admin" aria-label="Abrir administrador" className="inline-flex items-center gap-2 rounded-xl border border-grape/35 bg-grape/15 p-2.5 text-[#b9a9ff] transition hover:border-grape/65 hover:text-white lg:hidden motion-reduce:transition-none"><ShieldCheck size={17}/></Link> : null}<button onClick={signOut} aria-label="Cerrar sesión" title="Cerrar sesión" className="rounded-xl p-2.5 text-white/45 transition hover:bg-white/[.06] hover:text-white motion-reduce:transition-none"><LogOut size={19}/></button></nav></div></header>
