...
const KEY = "multilinks-profile";
export default function Dashboard() {
  const [profile, setProfile] = useState<Profile>(demoProfile);
  const [saved, setSaved] = useState(false);
  useEffect(() => { const value = localStorage.getItem(KEY); if (value) setProfile(JSON.parse(value)); }, []);
  const save = () => { localStorage.setItem(KEY, JSON.stringify(profile)); setSaved(true); setTimeout(() => setSaved(false), 1500); };
