...
import { ChevronDown, Crown, Eye, GripVertical, ImagePlus, LogOut, MousePointerClick, Plus, Settings, ShieldCheck, Trash2 } from "lucide-react";
...
import { decodeStoredBackground, encodeStoredBackground, premiumBackgrounds, premiumBackgroundStyle } from "@/lib/profile-backgrounds";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      if (dbProfile) {
        const storedBackground = decodeStoredBackground(dbProfile.background_color);
        setProfile({
  ...
  async function save() {
    ...
    if (!isPro && (profile.theme === "neon" || profile.backgroundPreset)) { setMessage("Los temas y fondos premium son exclusivos de MultiLinks Pro."); return; }
    setSaving(true);
    setMessage("");
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    ...
    const { error: profileError } = await supabase.from("profiles").upsert({
      ...
      theme: profile.theme,
      background_color: encodeStoredBackground(profile.backgroundColor, profile.backgroundPreset),
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#090b0d] text-white">
    ...
    <header className="relative border-b border-white/10 bg-[#0d1014]/80 px-4 py-3 backdrop-blur-xl lg:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center justify-between"><Link href="/" aria-label="Volver al inicio" className="text-white"><Logo/></Link><button onClick={signOut} aria-label="Cerrar sesión" className="rounded-xl p-2 text-white/45 transition hover:bg-white/[.06] hover:text-white sm:hidden motion-reduce:transition-none"><LogOut size={19}/></button></div><nav aria-label="Herramientas de la cuenta" className="flex w-full items-center gap-2 sm:w-auto"><Link href={`/${profile.username}`} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-lime px-3 py-2.5 text-sm font-black text-ink transition hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(201,255,88,.15)] sm:flex-none motion-reduce:transform-none motion-reduce:transition-none"><Eye size={17}/><span className="sm:hidden lg:inline">Perfil</span><span className="hidden sm:inline lg:hidden">Ver</span></Link><Link href="/dashboard/ajustes" className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/[.045] px-3 py-2.5 text-sm font-bold text-white/70 transition hover:border-lime/45 hover:text-lime sm:flex-none motion-reduce:transition-none"><Settings size={17}/><span>Ajustes</span></Link>{isAdmin ? <Link href="/admin" className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-grape/35 bg-grape/15 px-3 py-2.5 text-sm font-black text-[#b9a9ff] transition hover:border-grape/65 hover:text-white sm:flex-none motion-reduce:transition-none"><ShieldCheck size={17}/><span className="sm:hidden lg:inline">Admin</span><span className="hidden sm:inline lg:hidden">Admin</span></Link> : null}<button onClick={signOut} aria-label="Cerrar sesión" title="Cerrar sesión" className="hidden rounded-xl p-2.5 text-white/45 transition hover:bg-white/[.06] hover:text-white sm:block motion-reduce:transition-none"><LogOut size={19}/></button></nav></div></header>
    <div className="relative mx-auto grid max-w-7xl gap-8 px-5 py-8 lg:grid-cols-[1fr_410px]">
      <section>
        ...
        <div className="mt-6 rounded-[2rem] border border-white/15 bg-[#101318]/95 p-6 shadow-[0_24px_75px_rgba(0,0,0,.30)]">
          ...
          <div className="mt-5"><p className="text-sm font-bold text-white/75">Temas rápidos</p><div className="mt-3 flex flex-wrap gap-3">{(["lime", "violet", "sunset", "neon"] as const).map(theme => { const color = theme === "lime" ? "#c9ff58" : theme === "violet" ? "#8566ff" : theme === "sunset" ? "#ff7356" : "#0f1115"; const locked = theme === "neon" && !isPro; return <button key={theme} type="button" aria-label={locked ? "Tema Neon Dark, disponible en Pro" : `Tema ${theme}`} onClick={() => locked ? setMessage("Neon Dark es un tema exclusivo de MultiLinks Pro.") : setProfile({ ...profile, theme, backgroundPreset: undefined, backgroundColor: color, accentColor: theme === "neon" ? "#c6ff3d" : profile.accentColor })} className={`relative h-11 w-11 rounded-full border-2 transition motion-reduce:transition-none ${profile.theme === theme && !profile.backgroundPreset ? "border-lime ring-2 ring-lime/35 ring-offset-2 ring-offset-[#101318]" : "border-white/15 hover:border-white/35"}`} style={{ backgroundColor: color }}>{locked ? <span className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-lime text-ink"><Crown size={11}/></span> : null}</button>; })}</div></div>
          <div className="mt-6 border-t border-white/10 pt-6"><button type="button" onClick={() => setBackgroundsOpen(open => !open)} aria-expanded={backgroundsOpen} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[.035] px-4 py-4 text-left transition hover:border-lime/35 motion-reduce:transition-none"><span><span className="flex items-center gap-2 font-display text-sm font-black"><Crown size={16} className="text-lime"/> Fondos Premium</span><span className="mt-1 block text-xs text-white/35">8 diseños exclusivos · toca para desplegar</span></span><ChevronDown size={20} className={`text-white/45 transition motion-reduce:transition-none ${backgroundsOpen ? "rotate-180" : ""}`}/></button>{backgroundsOpen ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{premiumBackgrounds.map(background => { const selected = profile.backgroundPreset === background.id; return <button key={background.id} type="button" onClick={() => { if (!isPro) { setMessage("Los fondos Premium están disponibles con MultiLinks Pro."); return; } setProfile({ ...profile, theme: background.dark ? "neon" : "violet", backgroundPreset: background.id, backgroundColor: background.dark ? "#0f1115" : "#f7f4ed", accentColor: background.dark ? "#c6ff3d" : "#8566ff" }); }} className={`group relative overflow-hidden rounded-2xl border-2 text-left transition hover:-translate-y-0.5 motion-reduce:transform-none motion-reduce:transition-none ${selected ? "border-lime shadow-[0_0_24px_rgba(201,255,88,.18)]" : "border-white/10 hover:border-white/30"}`}><span className="block aspect-[9/13] bg-cover" style={premiumBackgroundStyle(background.id)}/><span className="absolute inset-x-0 bottom-0 bg-black/70 px-2 py-2 text-[10px] font-black text-white backdrop-blur-sm">{background.name}</span>{!isPro ? <span className="absolute right-2 top-2 grid h-6 w-6 place-items-center rounded-full bg-lime text-ink"><Crown size={13}/></span> : null}</button>; })}</div> : null}</div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2"><ColorField label="Color de fondo" value={profile.backgroundColor ?? "#c9ff58"} onChange={backgroundColor => setProfile({ ...profile, backgroundColor, backgroundPreset: undefined })}/><ColorField label="Color de acento" value={profile.accentColor ?? "#8566ff"} onChange={accentColor => setProfile({ ...profile, accentColor })}/></div>
