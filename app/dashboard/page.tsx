...
import { Eye, GripVertical, ImagePlus, LogOut, Plus, Trash2 } from "lucide-react";
...
type DbLink = { id: string; title: string; url: string; active: boolean };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      if (!user) return;
      const [{ data: dbProfile }, { data: dbLinks }] = await Promise.all([
        ...
        supabase.from("links").select("id,title,url,active").eq("profile_id", user.id).order("position"),
      ]);
      if (dbProfile) {
        setProfile({
  ...
  async function save() {
    ...
    const rows = profile.links.map((link, position) => ({ profile_id: user.id, title: link.title, url: link.url, active: link.active, position }));
