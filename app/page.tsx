...
export default function Home() {
  return <main className="min-h-screen overflow-hidden">
    ...
    <section className="mx-auto grid min-h-[78vh] max-w-6xl items-center gap-14 px-6 py-14 lg:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="relative mx-auto w-full max-w-[390px] rotate-[3deg] rounded-[42px] border-[3px] border-ink bg-ink p-3 shadow-[16px_16px_0_#c9ff58]"><div className="rounded-[30px] bg-[#8566ff] px-7 py-10 text-center text-white"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border-2 border-white bg-lime text-2xl font-black text-ink">ML</div><h2 className="mt-5 text-2xl font-black">Mi MultiLinks</h2><p className="mt-2 text-sm text-white/75">Contenido, proyectos y comunidad</p><div className="mt-8 space-y-3 text-left text-ink">{["Mi portafolio", "Mis redes sociales", "Mi comunidad"].map(text => <div key={text} className="flex items-center justify-between rounded-2xl border-2 border-ink bg-white px-5 py-4 font-bold shadow-[3px_3px_0_#151515]">{text}<ArrowUpRight size={18}/></div>)}</div></div></div>
    </section>
  </main>;
}
