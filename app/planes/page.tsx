...
function PlanCard({ title, price, subtitle, features, featured = false, action }: { title: string; price: string; subtitle: string; features: string[]; featured?: boolean; action?: React.ReactNode }) {
  return <article className={`relative border-[3px] border-ink p-7 shadow-hard-lg ${featured ? "bg-ink text-white" : "bg-white"}`}>{featured ? <span className="absolute -top-5 right-5 inline-flex items-center gap-2 border-2 border-ink bg-white px-3 py-2 text-xs font-black text-ink shadow-hard"><Crown size={15}/> RECOMENDADO</span> : null}<h2 className="font-display text-xl font-black">{title}</h2><p className="mt-4 font-display text-3xl font-black">{price}</p><p className={`mt-1 text-xs font-bold ${featured ? "text-white/55" : "text-black/45"}`}>{subtitle}</p><ul className="my-7 space-y-3">{features.map(feature => <li key={feature} className="flex items-center gap-3 text-sm font-semibold"><span className={`grid h-6 w-6 shrink-0 place-items-center border-2 ${featured ? "border-white/30 bg-white/10 text-lime" : "border-ink bg-cream text-ink"}`}><Check size={14}/></span>{feature}</li>)}</ul>{action}</article>;
}
