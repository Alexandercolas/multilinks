...
export default async function PlansPage({ searchParams }: { searchParams: Promise<{ checkout?: string }> }) {
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#090b0d] px-5 py-8 text-white sm:py-12">
    ...
    <div className="grid gap-7 lg:grid-cols-3">
      <PlanCard title="Gratis" price="US$0" subtitle="para siempre" features={["Hasta 5 enlaces activos", "1 perfil", "3 temas básicos", "Estadísticas de 7 días", "Marca MultiLinks"]}/>
      <PlanCard title="Pro mensual" price="US$3.50" subtitle="cada mes" featured features={["Hasta 100 enlaces activos", "Hasta 3 perfiles", "Todos los temas", "Historial completo", "Sin marca MultiLinks", "Soporte prioritario"]} action={!isPro && user ? <CheckoutButton interval="monthly" userId={user.id}/> : undefined}/>
      <PlanCard title="Pro anual" price="US$39.99" subtitle="cada año · ahorras US$2.01" features={["Todo lo incluido en Pro", "Hasta 100 enlaces activos", "Hasta 3 perfiles", "Historial completo", "Sin marca MultiLinks", "Mejor precio del año"]} action={!isPro && user ? <CheckoutButton interval="annual" userId={user.id}/> : undefined}/>
    </div>
