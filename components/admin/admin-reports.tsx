...
export function AdminReports({ reports }: { reports: AdminReportRow[] }) {
  return <section id="reportes" className="mt-8 border-[3px] border-ink bg-grape text-white shadow-hard-lg">
    <div className="flex items-center justify-between gap-4 border-b-[3px] border-ink p-5"><div><p className="font-display text-xs font-black uppercase tracking-[.14em] text-lime">Moderación</p><h2 className="mt-1 font-display text-xl font-black">Reportes pendientes</h2></div><span className="grid h-12 w-12 place-items-center border-2 border-ink bg-lime text-ink shadow-[3px_3px_0_#151515]"><Flag/></span></div>
    ...
    {reports.length === 0 ? <div className="p-10 text-center"><p className="font-display text-lg font-black">Todo limpio por aquí.</p><p className="mt-2 text-sm text-white/70">No hay reportes pendientes de revisión.</p></div> : null}
  </section>;
}
