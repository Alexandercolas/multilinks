  ...
  return <section id="usuarios" className="mt-8 overflow-hidden rounded-[2rem] border border-white/15 bg-[#101318]/95 text-white shadow-[0_28px_90px_rgba(0,0,0,.35)]">
    <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-center sm:justify-between">
      ...
      <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.045] px-3 py-2 text-white/55 focus-within:border-lime/70 focus-within:text-lime">
        ...
        <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Correo, nombre o usuario" className="w-full min-w-0 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-white/25 sm:w-64"/>
      </label>
    </div>
    <div className="overflow-x-auto"><table className="w-full min-w-[960px] text-left text-sm">
      ...
      <tbody>{filteredUsers.map((user) => <tr key={user.id} className="border-b border-white/10 align-top transition last:border-b-0 hover:bg-white/[.025] motion-reduce:transition-none">
    ...
    {filteredUsers.length === 0 ? <p className="border-t border-white/10 p-8 text-center font-bold text-white/45">No encontramos usuarios con esa búsqueda.</p> : null}
  </section>;
}
